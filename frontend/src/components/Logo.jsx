...
export default function Logo({ size = 'md', linkTo = null }) {
  const sizes = {
    sm: { box: 'w-8 h-8', icon: 14, title: 'text-sm', sub: 'text-[9px]' },
    md: { box: 'w-10 h-10', icon: 18, title: 'text-base', sub: 'text-[10px]' },
    lg: { box: 'w-14 h-14', icon: 22, title: 'text-xl', sub: 'text-xs' },
  };
  const s = sizes[size];
  ...
  const content = (
    <div className="flex items-center gap-3">
      <div className={`${s.box} bg-neon flex items-center justify-center rounded-sm`}>
        <Activity strokeWidth={2.5} size={s.icon} className="text-black" />
      </div>
