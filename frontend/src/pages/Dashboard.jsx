...
export default function Dashboard() {
  ...
  const [active, setActive] = useState(getActiveMatch());
  ...
  }, [navigate, sessionUser?.id]);  // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleClearActiveOnly = () => {
    if (!window.confirm('Cancelar o jogo em curso (se existir)? Os jogos do histórico mantêm-se.')) return;
    clearActiveMatch();
    setActive(null);
    toast.message('JOGO EM CURSO CANCELADO');
  };
  ...
  const totalMinutesPlayed = matches.reduce((s, m) => s + (m.total_duration || 0), 0);
      ...
      <header className="border-b border-white/10 px-6 lg:px-10 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-5">
          <div className="hidden sm:block text-right">
            ...
            <div className="text-sm text-white">{team.coach || sessionUser.name}</div>
          </div>
      ...
      <main className="flex-1 px-6 lg:px-10 py-10 max-w-7xl mx-auto w-full">
        <div className="mb-10 fade-up">
          ...
          <p className="text-sm text-white/55 mt-4 max-w-xl">
            Bem-vindo, {team.coach || sessionUser.name}. Inicia um novo jogo ou consulta o histórico da equipa.
          </p>
        </div>
        ...
        {active && (
          <div className="border border-neon/40 bg-[#161b05] rounded-sm p-5 lg:p-6 mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-4 fade-up">
            ...
            <Link to="/monitor" className="bg-neon text-black font-display text-lg uppercase tracking-wider px-7 py-3 rounded-sm hover:bg-[#bbdc0d] transition-colors inline-flex items-center gap-2 self-start lg:self-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {cards.map((c) => {
            const Icon = c.icon;
            return (
              <Link key={c.title} to={c.to} className={`group border rounded-sm p-6 transition-all hover:border-neon ${c.featured ? 'border-neon/40 bg-[#0f1305]' : 'border-white/10 bg-[#0f0f0f]'}`}>
                <div className="flex items-start justify-between mb-5">
                  <div className={`w-12 h-12 flex items-center justify-center rounded-sm ${c.featured ? 'bg-neon text-black' : 'bg-white/5 text-neon'}`}>
                    <Icon size={22} strokeWidth={2} />
                  </div>
                  <ChevronRight size={18} className="text-white/30 group-hover:text-neon group-hover:translate-x-1 transition-all" />
                </div>
        ...
        <div className="border border-red-500/20 bg-red-500/5 rounded-sm p-5">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-sm flex items-center justify-center" style={{ backgroundColor: team.color || '#d4ff1a' }}>
              ...
              <div>
                ...
                <div className="text-[10px] tracking-label uppercase text-white/50">
                  Criada em {new Date(team.created_at).toLocaleDateString('pt-PT')}
                </div>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              {active && (
                <button onClick={handleClearActiveOnly} className="text-xs uppercase tracking-label bg-white/5 border border-white/10 text-white/70 hover:text-orange-400 hover:border-orange-400/40 flex items-center gap-2 px-4 py-2 rounded-sm transition-colors">
                  <RotateCcw size={12} /> Cancelar Jogo em Curso
                </button>
              )}
              <button onClick={handleResetTeam} className="text-xs uppercase tracking-label bg-red-500/15 border border-red-500/40 text-red-300 hover:bg-red-500/25 flex items-center gap-2 px-4 py-2 rounded-sm transition-colors font-semibold">
